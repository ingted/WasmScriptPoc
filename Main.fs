...
module Compiler = 
    ...
    let run (compiler: Compiler) (source: string) = async {
        ...
        // Update source files in options? GetProjectOptionsFromCommandLineArgs should be enough if we re-call it or just rely on file system
        // But ParseAndCheckProject reads from file usually.
        
        let! (errors, exitCode) = compiler.Checker.Compile(Array.append compiler.Options.OtherOptions compiler.Options.SourceFiles)
